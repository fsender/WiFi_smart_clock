...
uint8_t wait_ready_s(plyr * player){ //等待准备
  ...
  while(1){
   ...
   if(digitalRead(D1)==0 && digitalRead(D4)==0) return 1;
   if(prpflag==2) break;
  }
  return 0;
}
    ...
    if(winner==0){
      ...
      do{
        ...
        while(digitalRead(D3)==1) {
            ...
            u8g2.setColorIndex(1);
          draw_card(&player[yourID].cards,0,49,15*(digitalRead(D4)==0 ||digitalRead(D1)==0));
          yield();
        }
      ...
      if(s>base_score) {
        base_score=s;
        u8g2.drawUTF8(51,37,ch_calling[s]);
        amax=0;
      }
      else u8g2.drawUTF8(51,37,ch_calling[0]);
      u8g2.sendBuffer();
    }
    else {
...
uint8_t serve_card(plyr * player,pcards *pool,uint8_t sendOnly){ //处理打来的牌
  ...
  if(p_holder==0 && !sendOnly){//自己出牌
    ...
    while(!playr[p_holder-1].available()){
        ...
        if(digitalRead(D4)==0 ||digitalRead(D1)==0){
        ...
        draw_card(&player[0].cards,0,49,15*(digitalRead(D4)==0 ||digitalRead(D1)==0));
        }
...
uint8_t start_connect_to(const char *ip,uint32_t h_port,plyr *player){//连接到房主
  ...
  if(gotid==1 || gotid==2) {
    ...
    char idn[2]={0};
    idn[0]=gotid+48;
    drawList("获得玩家ID成功",playername,idn,"等待其他玩家进入游戏",1);
  //等待其他玩家加入游戏，当所有玩家加入且主机已准备后，主机将发送所有玩家昵称信息，此时不用回复
    while(!playr[0].available() && (digitalRead(D3) || digitalRead(D4))) {
    if(digitalRead(D1)==0){
      playr[0].stop();
      delay(200);
      return 0;
...
uint8_t wait_ready(){ //等待准备
  ...
  while(!playr[0].available()) {
    yield();
    if(millis()-prepareTime>10000) return 1;
  }
  delay(10);
  if(playr[0].read()=='r') {
    rtnFlag=0; //成功回应
  ...
  if(playr[0].available()){
   ...
   if(preread=='l'){
    ...
    if(buff[2]<buff[1]){
      ...
      else if(buff[0]-48==(yourID+1)%3) u8g2.drawUTF8(51,37,ch_calling[0]);
      ...
      else if(buff[0]-48==(yourID+1)%3) u8g2.drawUTF8(51,37,ch_calling[base_score]);
...
//  return 0;
//}
void draw_card(pcards *To, uint8_t x, uint8_t y, uint8_t xpage, uint8_t n, uint8_t selecting, const uint8_t *selected ){
  ...
  for(;;){
    ...
    if(digitalRead(D3)==0){//选择
      if(cur<240){
        ...
        u8g2.setColorIndex(1);
        u8g2.setFont(u8g2_font_4x6_tr);
        u8g2.setCursor(0,55); u8g2.print("Current Scores:");
        u8g2.setCursor(64,55); u8g2.print(playing[0].name);
        u8g2.print(": "); u8g2.print(playing[0].score);
        u8g2.setCursor(0,63); u8g2.print(playing[1].name);
        u8g2.print(": "); u8g2.print(playing[1].score);
        u8g2.setCursor(64,63); u8g2.print(playing[2].name);
...
void disp_status(const char* st,uint8_t ry){
      u8g2.setFont(u8g2_font_wqy12_t_gb2312);
      u8g2.setColorIndex(0);
      if(ry!=37) u8g2.drawBox(0,ry-15,128,16);
      u8g2.setColorIndex(1);
      if(ry==37) u8g2.drawUTF8(28,ry,st);
      else u8g2.drawUTF8(0,ry,st);
      u8g2.sendBuffer();
}
...
void dodizhu(){
  ...
  #endif  */
#ifdef DBG_VERSION
  DBG_MODE =(!digitalRead(D4));
#endif
  uint8_t ddzmode=listdraw("斗地主OL 局域网版",ddz_modes,3);
  ...
  while(!quit_flag){
    ...
    if(yourID>=3){//返回>3代表游戏进行中但是你掉线了
      ...
      if((eventing=sync_data(players,pool))==0) {//同步信息，若失败(返回0)则退出游戏
        drawList("同步消息失败","服务器地址:",play_ip,"可以尝试重新连接");
        break;
        //eventing : 0.同步失败
        //1.等待准备，此时可以加入游戏
        //2.等待叫分
    ...
    if(eventing==1 || eventing==0){ //表示游戏未开始，正常流程
      ...
      while(1){
        ...
        while(digitalRead(D3)==1 &&digitalRead(D1)==1) yield();
        if(digitalRead(D1)==0) return;
        while(digitalRead(D3)==0) yield();
        delay(24);
        send_ready();
      }
      ...
      while(1){
        ...
        do{
          ...
          while(digitalRead(D3)==1) {
            ...
            u8g2.drawBox(0,48,128,16);
            u8g2.setColorIndex(1);
            draw_card(&players[yourID].cards,0,49,15*(digitalRead(D4)==0 ||digitalRead(D1)==0));
            yield();
          }
          while(digitalRead(D3)==0) yield();
          delay(24);
    ...
    while(eventing==3 || eventing==0){
      ...
      if(get_order(players,pool)){//1代表成功获得出牌权
        ...
        if(digitalRead(D3)==0 && digitalRead(D4)==0) continue;
      }
        //检验是否发送完成
        if(send_flag){
